...
import { Movie, MovieImage, MovieImageItem, MovieKeywordItem } from '@/interfaces/movie'
import { getMovieByID, getMovieImages, getMovieKeywords } from '@/pages/api/movie'
...
const MovieDetails = () => {
  ...
  const [movieKeywords, setMovieKeywords] = useState<MovieKeywordItem | null>(null)
  ...
  return (
    <>
    <div className='grid grid-rows-[1fr_2fr] gap-4 md:grid-rows-[1fr_2fr] md:gap-4 lg:grid-rows-[85vh_1fr] lg:gap-4 py-8 px-8'>
      ...
      <div className='grid grid-cols-1  gap-4 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-[400_1fr]'>
        ...
        <div className='w-full md:h-[88vh] lg:h-[full]'>
          <Image
            src={
              movie.poster_path
        ...
        <div className='h-full md:h-[88vh]'>
          {/*Overview...*/}
          <div>
         ...
         <div className='mt-4'>
          <button className='bg-green-200 text-green-800 py-2 px-4 rounded-full cursor-pointer'>Add to favorites</button>
         </div>
          ...
          {activeTab === 'backdrops' && (
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
              {movieImages.backdrops && movieImages.backdrops.length > 0 ? (
                movieImages.backdrops.slice(0, 9).map((image: MovieImageItem, index: number) => (
                  <div
                ...
                <p className='text-gray-500 col-span-full'>No backdrops available</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'posters' && (
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4'>
              {movieImages.posters && movieImages.posters.length > 0 ? (
                movieImages.posters.slice(0, 10).map((image: MovieImageItem, index: number) => (
                  <div
                ...
                <p className='text-gray-500 col-span-full'>No posters available</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'logos' && (
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
              {movieImages.logos && movieImages.logos.length > 0 ? (
                movieImages.logos.slice(0, 8).map((image: MovieImageItem, index: number) => (
                  <div
                ...
                <p className='text-gray-500 col-span-full'>No logos available</p>
              )}
            </div>
          )}
        </div>
